...
async function lifecycleTest(store: RedisStore, client: any): Promise<void> {
  ...
  const promiseStoreSet = promisify(store.set).bind(store);
  const promiseStoreGet = promisify(store.get).bind(store);
  const promiseStoreDestroy = promisify(store.destroy).bind(store);

  // await store.set("123", sess);
  await promiseStoreSet("123", sess);

  // res = await store.get("123");
  res = await promiseStoreGet("123");
  ...
  await promiseStoreSet("456", { cookie: { originalMaxAge: null, expires } });
  ...
  res = await promiseStoreSet("789", {
    cookie: { originalMaxAge: null, expires },
  });
  ...
  await promiseStoreSet("789", { cookie: { originalMaxAge: null, expires } });
...
async function load(store: RedisStore, count: number) {
  ...
  for (let sid = 0; sid < count; sid++) {
    ...
    await promiseStoreSet("s" + sid, { cookie });
  }
}
